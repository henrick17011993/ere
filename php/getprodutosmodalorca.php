...
if (isset($_GET['idorcamento'])) {
    ...
    if (!empty($produtosAgrupados)) {
        foreach ($produtosAgrupados as $tipo => $produtos) {
            ...
            echo '<h2 style="text-align: center; background-color: white; width: 100%; color: #3498db; border-radius: 10px; padding: 5px;">' . htmlspecialchars($tipo, ENT_QUOTES, 'UTF-8') . '</h2>';
            echo '<table class="tabela">';
            echo '<thead><tr>';
            echo '<th>Imagem</th><th>Tipo</th><th>Modelo</th><th>Cor</th><th>Valor</th><th>Quant</th>';
            echo '</tr></thead><tbody>';
            foreach ($produtos as $produto) {
                $nova_imagem = isset($produto["imagem"]) ? htmlspecialchars($produto["imagem"], ENT_QUOTES, 'UTF-8') : 'uploads/logoere.png';
                $imagemurl = str_replace("http://localhost/everton", "/everton", $nova_imagem);
                ...
                echo '<tr>';
                echo '<td><div class="image-container"><img src="' . $imagem . '" alt="Imagem" onerror="this.onerror=null; this.src=\'uploads/logoere.png\';"></div></td>';
